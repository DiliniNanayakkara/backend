...
app.post("/addtoorder", (req, res) => {
  ...
  app.post("/setdelivery", (req, res) => {
    ...
    db.query(
      ...
      [user, total, firstname, lastname, address, phone, email, city, date],
      (err, result) => {
        if (err) {
          console.log(err);
        } else {
...
app.post("/quantityupdate", (req, res) => {
  ...
  db.query(
    " UPDATE tools SET tool_quantity = (tool_quantity - '" +
      quantity +
      "' )  WHERE tool_name = '" +
      art +
      "'",
...
app.post("/ordercomplete", (req, res) => {
  const { user } = req.body;
  db.query("DELETE FROM cart WHERE user = '" + user + "' ", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
